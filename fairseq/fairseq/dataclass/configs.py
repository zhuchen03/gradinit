...
@dataclass
class GradInitParams(FairseqDataclass):
    # optiosn for GradInit
    use_gradinit: bool = field(
        default=False, metadata={"help": "whether to use gradinit"}
    )
    resume_init_chk: str = field(
        default="", metadata={"help": "resume from existing gradinit checkpoint and start training"}
    )
    gradinit_min_scale: float = field(
        default=0.01, metadata={"help": "minimum rescaling allowed by gradinit"}
    )
    gradinit_eta: float = field(
        default=5e-4, metadata={"help": "step size of gradinit"}
    )
    gradinit_grad_clip: float = field(
        default=1, metadata={"help": "gradient clipping for gradinit"}
    )
    gradinit_max_tokens: int = field(
        default=6000, metadata={"help": "max number of tokens to create the overlapping batches"}
    )
    gradinit_iters: int = field(
        default=780, metadata={"help": "max number of tokens to create the overlapping batches"}
    )
    gradinit_alg: str = field(
        default='adam', metadata={"help": "the target optimization algorithm after gradinit"}
    )
    gradinit_gamma: float = field(
        default=1000, metadata={"help": "the norm constraint of gradinit"}
    )
    gradinit_lr: float = field(
        default=1e-3, metadata={"help": "learning rate of gradinit on the scales"}
    )
    gradinit_ln_lr: float = field(
        default=-1, metadata={"help": "separate lr for LN"}
    )
    gradinit_max_scale: float = field(
        default=float('inf'), metadata={"help": "minimum rescaling allowed by gradinit"}
    )
    gradinit_beta1: float = field(
        default=0.9, metadata={"help": "beta1 of RescaleAdam"}
    )
    gradinit_beta2: float = field(
        default=0.999, metadata={"help": "beta1 of RescaleAdam"}
    )
...
class FairseqConfig(FairseqDataclass):
    ...
    gradinit: GradInitParams = GradInitParams()